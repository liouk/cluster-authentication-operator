...
func TestExternalOIDCController_oidcPreconditionsFulfilled(t *testing.T) {
	for _, tt := range []struct {
		name string
		...
		{
			name:      "rolebindingrestrictions lister error",
			crdExists: true,
			rbrListReaction: func(_ k8stesting.Action) (bool, runtime.Object, error) {
				return true, nil, fmt.Errorf("rolebindingrestrictions lister error")
			},
			expectFulfilled: false,
			expectMessage:   false,
			expectError:     true,
		},
		{
			name:            "no rolebindingrestrictions exist",
			crdExists:       true,
			expectFulfilled: true,
			expectMessage:   false,
			expectError:     false,
		...
		{
			name:      "rolebindingrestrictions exist",
			crdExists: true,
			rbrs: []*authzv1.RoleBindingRestriction{
				{
					ObjectMeta: metav1.ObjectMeta{
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			rbrs := make([]runtime.Object, 0, len(tt.rbrs))
			for _, rbr := range tt.rbrs {
				rbrs = append(rbrs, rbr)
			}

			clientset := fakeauthz.NewSimpleClientset(rbrs...)
			if tt.rbrListReaction != nil {
				clientset.PrependReactor("list", "rolebindingrestrictions", tt.rbrListReaction)
			}

			c := externalOIDCController{
				crdLister:   apiextensionslister.NewCustomResourceDefinitionLister(crdIndexer),
				authzClient: clientset,
			}

			fulfilled, msg, err := c.oidcPreconditionsFulfilled(context.TODO())

			if tt.expectFulfilled != fulfilled {
				t.Errorf("expected preconditions fulfilled: %v; got: %v", tt.expectFulfilled, fulfilled)
			}

			if tt.expectMessage != (len(msg) > 0) {
				t.Errorf("expected message: %v; got: %s", tt.expectMessage, msg)
			}
